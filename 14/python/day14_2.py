...
class Cave:
    ...
    def create_cave(self):
        ...
        self.max_y = max([point.y for rockpath in self.rockpaths for point in rockpath.points])
        self.add_rockpath(RockPath(f'{500 - (self.max_y//2 + 2) * 2},{self.max_y + 2} -> {500 + (self.max_y//2 + 2) * 2},{self.max_y + 2}'))
        ...
        self.sand_at_source = False

    def drop_sand(self):
        while not self.sand_at_source:
            self.sand_step()
        self.draw_cave()

    def sand_step(self):
        if self.sand_at_rest:
            if self.sand_current_position == self.sand_drop_point:
                self.sand_at_source = True
        ...
        if self.cave[self.sand_current_position.y + 1][self.sand_current_position.x] == '.':
